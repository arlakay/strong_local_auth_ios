...
+ (instancetype)makeWithBiometricOnly:(BOOL )biometricOnly
  ...
  pigeonResult.strongMode = strongMode;
...
+ (FLAAuthOptions *)fromList:(NSArray *)list {
  ...
  pigeonResult.strongMode = [GetNullableObjectAtIndex(list, 1) boolValue];
  pigeonResult.sticky = [GetNullableObjectAtIndex(list, 2) boolValue];
  pigeonResult.useErrorDialogs = [GetNullableObjectAtIndex(list, 3) boolValue];
  return pigeonResult;
}
+ (nullable FLAAuthOptions *)nullableFromList:(NSArray *)list {
...
NSObject<FlutterMessageCodec> *FLALocalAuthApiGetCodec(void) {
  ...
  dispatch_once(&sPred, ^{
    FLALocalAuthApiCodecReaderWriter *readerWriter = [[FLALocalAuthApiCodecReaderWriter alloc] init];
    sSharedObject = [FlutterStandardMessageCodec codecWithReaderWriter:readerWriter];
  });
  return sSharedObject;
}

void SetUpFLALocalAuthApi(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FLALocalAuthApi> *api) {
  /// Returns true if this device supports authentication.
  {
    ...
    if (api) {
      NSCAssert([api respondsToSelector:@selector(isDeviceSupportedWithError:)], @"FLALocalAuthApi api (%@) doesn't respond to @selector(isDeviceSupportedWithError:)", api);
  ...
  /// any biometrics are enrolled or not.
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        ...
        codec:FLALocalAuthApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(deviceCanSupportBiometricsWithError:)], @"FLALocalAuthApi api (%@) doesn't respond to @selector(deviceCanSupportBiometricsWithError:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        FlutterError *error;
  ...
  /// without additional setup.
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        ...
        codec:FLALocalAuthApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(getEnrolledBiometricsWithError:)], @"FLALocalAuthApi api (%@) doesn't respond to @selector(getEnrolledBiometricsWithError:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        FlutterError *error;
  ...
  /// [strings] for any UI.
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        ...
        codec:FLALocalAuthApiGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(authenticateWithOptions:strings:completion:)], @"FLALocalAuthApi api (%@) doesn't respond to @selector(authenticateWithOptions:strings:completion:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        FLAAuthOptions *arg_options = GetNullableObjectAtIndex(args, 0);
        FLAAuthStrings *arg_strings = GetNullableObjectAtIndex(args, 1);
        [api authenticateWithOptions:arg_options strings:arg_strings completion:^(FLAAuthResultDetails *_Nullable output, FlutterError *_Nullable error) {
          callback(wrapResult(output, error));
        }];
      }];
    } else {
